...
function TrafficStatusReport({ accesos, vialidades, rutasFiscales }) {
  ...
  const cleanReportText = (text) => String(text ?? "")
    ...
    .replace(/\s+/g, " ")
    .trim();
  const cleanPdfText = cleanReportText;

  const refreshData = useCallback(async () => {
    setLoading(true);
    try {
      const [{ data: terminalsData }, { data: patiosData }, { data: carrilesData }] = await Promise.all([
  ...
  const terminalMap = Object.fromEntries((remote.terminals || []).map(r => [r.id, r]));
  const patioMap = Object.fromEntries((remote.patios || []).map(r => [r.id, r]));
  ...
  const reportGroups = useMemo(() => {
    ...
    if (include.terminales) {
      const all = [...TERMINALS_NORTE.map(t => ({ ...t, zona: "Norte" })), ...TERMINALS_SUR.map(t => ({ ...t, zona: "Sur" }))];
      groups.push({
        id: "terminales",
        title: "Terminales",
        items: all.map(t => {
  ...
  const totalItems = reportGroups.reduce((sum, g) => sum + g.items.length, 0);
  ...
  const drawWatermark = (doc) => {
    try {
      const pageWm = doc.internal.pageSize.getWidth();
  ...
  const generatePdf = async () => {
    if (!totalItems) return notify("Selecciona al menos una sección", "#f97316");
    setBusyPdf(true);
    try {
      ...
      console.error(e);
      notify("No se pudo generar el PDF", "#ef4444");
    } finally {
      setBusyPdf(false);
    }
  };
          ...
          {reportGroups.map(g => (
            <div key={g.id} style={{ marginBottom:"14px" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"6px 0", borderBottom:"1px solid rgba(56,189,248,.22)", marginBottom:"4px" }}>
                <div style={{ color:"#38bdf8", fontFamily:getFont(theme,"secondary"), fontSize:"13px", fontWeight:"900", textTransform:"uppercase", letterSpacing:".7px" }}>{cleanReportText(g.title)}</div>
  ...
  return (
    <div style={{ marginBottom: "14px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "6px" }}>
        <span style={{ fontFamily: getFont(theme, "secondary"), fontSize: "10px", color: "rgba(255,255,255,0.55)", fontWeight: "600" }}>{label}</span>
        <span style={{ fontFamily: "monospace", fontSize: "12px", color, fontWeight: "700" }}>{Number(value).toLocaleString("es-MX")} {unit}</span>
      </div>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: "6px" }}>
              {TIPOS_CARGA_FLETE.map(t => (
                <button key={t.id} onClick={() => setTipoCargaId(t.id)} style={{
                  ...
                  color: tipoCargaId === t.id ? "#a78bfa" : "rgba(255,255,255,0.45)",
                  fontFamily: getFont(theme, "secondary"), fontSize: "9px", fontWeight: "700",
                  cursor: "pointer", transition: "all 0.15s"
                }}>
                  {t.label}
                  {t.factor > 1 && <span style={{ color: "#f87171", fontSize: "8px", marginLeft: "4px" }}>+{Math.round((t.factor-1)*100)}%</span>}
                </button>
              ))}
            </div>
          </div>
