...
// ─── COMPONENTE: MENÚ FLOTANTE DE AUTENTICACIÓN ───────────────────────────────
function FloatingAuthMenu({ onOpenAuth, user, onLogout }) {
  ...
  // Auto-cerrar después de inactividad (5 segundos)
  useEffect(() => {
    if (isOpen) {
  ...
  // Resetear timeout al mover el mouse sobre el menú
    ...
    >
      ...
      <button
        ...
        style={{
          ...
          boxShadow: isOpen
            ? '0 8px 30px rgba(59,130,246,0.5), 0 0 0 4px rgba(59,130,246,0.2)'
        ...
        onMouseEnter={(e) => {
          if (!isOpen) {
            ...
            e.target.style.boxShadow = '0 8px 25px rgba(56,189,248,0.5)';
          }
        }}
        onMouseLeave={(e) => {
          if (!isOpen) {
...
function AuthModal({ mode, onClose, onSuccess, requiredAction = null }) {
  const [step, setStep] = useState(mode === "register" ? "form" : mode === "forgot" ? "forgot" : "login");
