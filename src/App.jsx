...
function SegundoAccesoTab() {
  ...
  useEffect(() => {
    sb.from(TABLA).select("*").eq("id", ROW_ID).single().then(({ data }) => {
      if (data?.data) setCarriles(prev => ({ ...mkSegundoIngreso(), ...prev, ...data.data }));
    });
    sb.from(TABLA).select("*").eq("id", ROW_ID_CF).single().then(({ data }) => {
      if (data?.data) setConfinada(prev => ({ ...mkConfinadaState(), ...prev, ...data.data }));
    });
    const chan = sb.channel("segundo-rt")
      .on("postgres_changes", { event: "*", schema: "public", table: TABLA }, ({ new: r }) => {
        if (r?.id === ROW_ID && r?.data) setCarriles(prev => ({ ...prev, ...r.data }));
        if (r?.id === ROW_ID_CF && r?.data) setConfinada(prev => ({ ...prev, ...r.data }));
      }).subscribe();
    return () => sb.removeChannel(chan);
  }, []);
  ...
  const updateIngreso = async (id, field, value) => {
    try {
      ...
      notify("✗ Error al actualizar", "#ef4444");
    }
  };
  ...
  const updateConfinada = async (id, field, value) => {
    try {
      ...
      console.error("Error al actualizar carril confinada:", error);
      notify("✗ Error al actualizar", "#ef4444");
    }
  };
    ...
    <div style={{ padding:"16px", paddingBottom:"80px", minHeight:"100vh" }}>
      ...
      {subTab === "segundo" && <>
        <div style={{ background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.15)", borderRadius:"12px", padding:"14px", marginBottom:"18px" }}>
          ...
          <div style={{ display:"flex", gap:"8px" }}>
            {SEGUNDO_CARRILES_INGRESO.map((c, i) => {
              ...
              const tc = tz === "Norte" ? "#38bdf8" : tz === "Sur" ? "#a78bfa" : "#fbbf24";
              return (
          ...
          <div style={{ display:"flex", justifyContent:"center", gap:"10px", marginTop:"10px", flexWrap:"wrap" }}>
            {[["#22c55e","LIBRE"],["#ef4444","SATURADO"],["#f59e0b","T. LENTO"],["#dc2626","T. DETENIDO"],["#38bdf8","ZONA NORTE"],["#a78bfa","ZONA SUR"],["#fbbf24","GENERAL"]].map(([c,l]) => (
        ...
        {SEGUNDO_CARRILES_INGRESO.map(carril => {
          ...
          const zonaColor = termObj?.zona === "Norte" ? "#38bdf8" : termObj?.zona === "Sur" ? "#a78bfa" : "#fbbf24";
            ...
            <div key={carril.id} style={{ background:"rgba(255,255,255,0.08)", backdropFilter:"blur(12px)", WebkitBackdropFilter:"blur(12px)", border:`1px solid ${st.saturado ? "#ef444466" : zonaColor+"44"}`, borderRadius:"12px", padding:"14px", marginBottom:"14px" }}>
              ...
              <div style={{ marginBottom:"8px" }}>
                ...
                <div style={{ display:"flex", gap:"5px", flexWrap:"wrap" }}>
                  {termsNorte.map(t => {
                    const btnColor = t.zona === "Norte" ? "#38bdf8" : t.zona === "Sur" ? "#a78bfa" : "#fbbf24";
                    return <button key={t.id} onClick={() => updateIngreso(carril.id,"terminal",t.id)} style={{ padding:"5px 10px", background: st.terminal===t.id?btnColor+"22":"#0a1628", border:`1px solid ${st.terminal===t.id?btnColor:"#1e3a5f"}`, borderRadius:"6px", color: st.terminal===t.id?btnColor:"#475569", fontFamily:getFont(theme, "secondary"), fontSize:"10px", cursor:"pointer", fontWeight: st.terminal===t.id?"700":"400" }}>{t.name}</button>;
                  })}
                </div>
              </div>
              <div style={{ marginBottom:"10px" }}>
      ...
      {subTab === "confinada" && <>
        ...
        <div style={{ background:"rgba(255,255,255,0.05)", border:"1px solid rgba(167,139,250,0.25)", borderRadius:"12px", padding:"14px", marginBottom:"18px" }}>
          ...
          <div style={{ display:"flex", gap:"8px" }}>
            {CONFINADA_CARRILES.map((c, i) => {
              ...
              const tz = getTermZona(st.terminal);
              const termColor = sinUso ? "#6b7280" : (tz === "Norte" ? "#38bdf8" : tz === "Sur" ? "#a78bfa" : "#fbbf24");
              ...
              return (
                <div key={c.id} style={{ flex:1, background:bc+"15", border:`2px solid ${bc}`, borderRadius:"10px", padding:"12px 6px", textAlign:"center", display:"flex", flexDirection:"column", gap:"6px", justifyContent:"center" }}>
                  ...
                  {sinUso
                    ...
                    : <div style={{ background:termColor+"22", border:`1px solid ${termColor}55`, borderRadius:"6px", padding:"4px 4px" }}>
                        <SlotText value={getTermName(st.terminal)} color={termColor} fontSize="clamp(10px,1.8vw,14px)" delay={i * 180} />
                      </div>
                  }
          ...
          <div style={{ display:"flex", justifyContent:"center", gap:"10px", marginTop:"10px", flexWrap:"wrap" }}>
            {[["#22c55e","LIBRE"],["#ef4444","SATURADO"],["#fbbf24","TRANSFERENCIA"],["#38bdf8","ZONA NORTE"],["#a78bfa","ZONA SUR"],["#fbbf24","GENERAL"],["#6b7280","SIN USO"]].map(([c,l]) => (
              <div key={l} style={{ display:"flex", alignItems:"center", gap:"3px" }}>
                <div style={{ width:"8px", height:"8px", background:c, borderRadius:"2px" }} />
                <span style={{ fontSize:"9px", color:"rgba(255,255,255,0.5)", fontFamily:getFont(theme, "secondary") }}>{l}</span>
              </div>
        ...
        {CONFINADA_CARRILES.map(carril => {
          ...
          const termObj = TODAS_TERMINALES.find(t => t.id === st.terminal);
          const termZona = termObj?.zona;
          const termColor = termZona === "Norte" ? "#38bdf8" : termZona === "Sur" ? "#a78bfa" : termZona === "Todas" ? "#fbbf24" : "#a78bfa";
          const expoOpt = SEGUNDO_TRAFICO_OPTS.find(o => o.id === (st.expo || "libre"));
          const expoContOpt = SEGUNDO_CONTENEDOR_OPTS.find(o => o.id === st.expo_contenedor);
          const impoOpt = SEGUNDO_TRAFICO_OPTS.find(o => o.id === (st.impo || "libre"));
          const borderColor = st.terminal==="sin_uso" ? "#6b7280" : st.transferencia ? "#fbbf24" : st.saturado ? "#ef4444" : termColor;
          ...
          return (
            <div key={carril.id} style={{ background:"rgba(255,255,255,0.08)", backdropFilter:"blur(12px)", WebkitBackdropFilter:"blur(12px)", border:`1px solid ${borderColor}44`, borderRadius:"12px", padding:"14px", marginBottom:"14px" }}>
              ...
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:"12px" }}>
                <div>
                  <div style={{ display:"flex", alignItems:"center", gap:"8px" }}>
                    <div style={{ background:termColor+"22", border:`1px solid ${termColor}44`, borderRadius:"6px", padding:"3px 10px", color:termColor, fontFamily:getFont(theme, "secondary"), fontSize:"13px", fontWeight:"700" }}>{carril.label}</div>
                    {st.terminal === "sin_uso"
                ...
                <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:"5px" }}>
                  ...
                  {isChanged && <button onClick={() => resetOneConfinada(carril.id)} style={{ padding:"3px 8px", background:termColor+"15", border:`1px solid ${termColor}44`, borderRadius:"5px", color:termColor, fontFamily:getFont(theme, "secondary"), fontSize:"10px", cursor:"pointer", fontWeight:"700" }}>✓ NORMAL</button>}
                </div>
              </div>

              {/* Terminal asignada */}
              <div style={{ background: st.terminal==="sin_uso" ? "#6b728011" : termColor+"11", border:`1px solid ${st.terminal==="sin_uso"?"#6b728033":termColor+"33"}`, borderRadius:"8px", padding:"10px 12px", marginBottom:"12px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                <div>
                  <div style={{ fontSize:"9px", color:"rgba(255,255,255,0.5)", fontFamily:getFont(theme, "secondary"), letterSpacing:"1px", marginBottom:"2px" }}>TERMINAL ASIGNADA HOY · {termZona === "Todas" ? "TODAS LAS ZONAS" : `ZONA ${termZona?.toUpperCase() || "SUR"}`}</div>
                  {st.terminal === "sin_uso"
                    ...
                    : <><div style={{ color:termColor, fontFamily:getFont(theme, "secondary"), fontWeight:"700", fontSize:"15px" }}>{termObj?.name}</div><div style={{ color:"rgba(255,255,255,0.4)", fontSize:"10px", marginTop:"1px" }}>Zona {termObj?.zona}</div></>
                  }
                </div>
...
function CarrilesTab() {
  ...
  useEffect(() => {
    sb.from(TABLA).select("*").eq("id", ROW_ID).single().then(({ data }) => {
      if (data?.data) setEstado(prev => ({ ...mkCarrilesState(), ...prev, ...data.data }));
    });
    const chan = sb.channel("carriles-rt")
      .on("postgres_changes", { event: "*", schema: "public", table: TABLA }, ({ new: r }) => {
        if (r?.id === ROW_ID && r?.data) setEstado(prev => ({ ...prev, ...r.data }));
      }).subscribe();
    return () => sb.removeChannel(chan);
  }, []);
