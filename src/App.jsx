...
// Función de seguridad para acceder a Supabase sin que la app truene
const getSupabase = () => {
...
function useAdminMode() {
  ...
  return { isAdmin, handleLogoTap, logout, AdminModal };
}
...
function TerminalesStatus() {
  ...
  if (loading) {
    return (
      <div style={{ padding: "40px", textAlign: "center" }}>
        ...
        <p style={{ color: "rgba(255,255,255,0.5)", fontSize: "14px" }}>Cargando terminales...</p>
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: "20px 16px" }}>
      <div style={{ textAlign: "center", marginBottom: "24px", padding: "20px", background: "linear-gradient(145deg, rgba(23,23,23,0.95) 0%, rgba(15,15,15,0.95) 100%)", borderRadius: "16px", border: "1px solid rgba(255,255,255,0.08)" }}>
        <div style={{ fontSize: "48px", marginBottom: "12px" }}>🏢</div>
        <h2 style={{ margin: 0, fontSize: "24px", fontWeight: "700", color: "#fff", fontFamily: "'Space Mono', monospace", marginBottom: "8px" }}>Estado de Terminales</h2>
        <p style={{ margin: 0, fontSize: "13px", color: "rgba(255,255,255,0.5)" }}>Información en tiempo real</p>
      </div>

      <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
        {terminals.map((terminal) => {
          const statusConfig = ESTATUS_CONFIG[terminal.estado] || { emoji: "⚪", label: "Desconocido", color: "#6b7280" };
          return (
            <div key={terminal.id} style={{ background: "linear-gradient(145deg, rgba(23,23,23,0.95) 0%, rgba(15,15,15,0.95) 100%)", borderRadius: "16px", padding: "20px", border: "1px solid rgba(255,255,255,0.08)", boxShadow: "0 8px 32px rgba(0,0,0,0.4)", position: "relative", overflow: "hidden" }}>
              <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: "4px", background: `linear-gradient(90deg, ${statusConfig.color}, transparent)`, opacity: 0.6 }} />
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" }}>
                <div>
                  <h3 style={{ margin: 0, fontSize: "18px", fontWeight: "700", color: "#fff", fontFamily: "'Space Mono', monospace" }}>{terminal.nombre}</h3>
                  <div style={{ fontSize: "11px", color: "rgba(255,255,255,0.5)", marginTop: "4px" }}>{terminal.ubicacion}</div>
                </div>
                <div style={{ fontSize: "32px", padding: "8px", borderRadius: "12px", background: "rgba(255,255,255,0.05)", border: `2px solid ${statusConfig.color}`, boxShadow: `0 0 20px ${statusConfig.color}40` }}>{statusConfig.emoji}</div>
              </div>
              <div style={{ background: "rgba(255,255,255,0.03)", borderRadius: "10px", padding: "12px" }}>
                <div style={{ fontSize: "13px", color: statusConfig.color, fontWeight: "700", marginBottom: "8px" }}>{statusConfig.label}</div>
                {terminal.observaciones && <div style={{ fontSize: "12px", color: "rgba(255,255,255,0.6)", lineHeight: "1.5" }}>{terminal.observaciones}</div>}
              </div>
              {terminal.ultima_actualizacion && (
                <div style={{ marginTop: "12px", fontSize: "11px", color: "rgba(255,255,255,0.4)", textAlign: "right" }}>
                  Última actualización: {new Date(terminal.ultima_actualizacion).toLocaleString("es-MX")}
                </div>
              )}
            </div>
          );
...
function LaneStatus() {
  const [lanes, setLanes] = useState({ pv: {}, p15: {}, zn: {} });
  const supabase = getSupabase();
  ...
  useEffect(() => {
    ...
    const fetchLanes = async () => {
      try {
        const { data } = await supabase.from("lane_assignments").select("*");
        if (data) {
    ...
    fetchLanes();
    const sub = supabase.channel("lanes").on("postgres_changes", { event: "*", schema: "public", table: "lane_assignments" }, fetchLanes).subscribe();
  ...
  return (
    <div style={{ padding: "20px 16px" }}>
      <div style={{ textAlign: "center", marginBottom: "24px", padding: "20px", background: "linear-gradient(145deg, rgba(23,23,23,0.95) 0%, rgba(15,15,15,0.95) 100%)", borderRadius: "16px", border: "1px solid rgba(255,255,255,0.08)" }}>
        ...
        <h2 style={{ margin: 0, fontSize: "24px", fontWeight: "700", color: "#fff", fontFamily: "'Space Mono', monospace", marginBottom: "8px" }}>Estado de Carriles</h2>
        <p style={{ margin: 0, fontSize: "13px", color: "rgba(255,255,255,0.5)" }}>Asignación actual por acceso</p>
      </div>

      <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
        {ACCESOS.map((acceso) => (
          <div key={acceso.id} style={{ background: "linear-gradient(145deg, rgba(23,23,23,0.95) 0%, rgba(15,15,15,0.95) 100%)", borderRadius: "16px", padding: "20px", border: "1px solid rgba(255,255,255,0.08)", boxShadow: "0 8px 32px rgba(0,0,0,0.4)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "16px" }}>
              <span style={{ fontSize: "28px" }}>{acceso.icon}</span>
              <h3 style={{ margin: 0, fontSize: "18px", fontWeight: "700", color: "#fff", fontFamily: "'Space Mono', monospace" }}>{acceso.nombre}</h3>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
              <div style={{ background: "rgba(16,185,129,0.1)", border: "1px solid rgba(16,185,129,0.3)", borderRadius: "12px", padding: "14px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span style={{ fontSize: "13px", color: "rgba(16,185,129,0.9)", fontWeight: "600" }}>📤 Exportación</span>
                  <span style={{ fontSize: "18px", fontWeight: "700", color: "#10b981", fontFamily: "'Space Mono', monospace" }}>{lanes[acceso.id]?.exportacion || "Sin asignar"}</span>
                </div>
              </div>
              <div style={{ background: "rgba(59,130,246,0.1)", border: "1px solid rgba(59,130,246,0.3)", borderRadius: "12px", padding: "14px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span style={{ fontSize: "13px", color: "rgba(59,130,246,0.9)", fontWeight: "600" }}>📥 Importación</span>
                  <span style={{ fontSize: "18px", fontWeight: "700", color: "#3b82f6", fontFamily: "'Space Mono', monospace" }}>{lanes[acceso.id]?.importacion || "Sin asignar"}</span>
                </div>
              </div>
            </div>
          </div>
...
function Noticias() {
  ...
  useEffect(() => {
    ...
    const fetchNews = async () => {
      try {
        const { data } = await supabase.from("noticias").select("*").order("fecha_publicacion", { ascending: false }).limit(20);
    ...
    fetchNews();
    const sub = supabase.channel("news").on("postgres_changes", { event: "*", schema: "public", table: "noticias" }, fetchNews).subscribe();
    return () => supabase.removeChannel(sub);
  }, [supabase]);

  if (loading) {
    return <div style={{ padding: "40px", textAlign: "center" }}><div style={{ fontSize: "48px" }}>⏳</div></div>;
  }

  return (
    <div style={{ padding: "20px 16px" }}>
      {news.map((item) => (
        <div key={item.id} style={{ background: "rgba(23,23,23,0.95)", borderRadius: "16px", padding: "20px", marginBottom: "16px", border: "1px solid rgba(255,255,255,0.08)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "12px" }}>
            <h3 style={{ margin: 0, fontSize: "16px", fontWeight: "700", color: "#fff" }}>{item.titulo}</h3>
            <span>{item.icono || "📌"}</span>
          </div>
          <p style={{ margin: 0, fontSize: "14px", color: "rgba(255,255,255,0.7)", lineHeight: "1.6" }}>{item.contenido}</p>
        </div>
      ))}
    </div>
  );
...
function InfoTab() {
  return (
    <div style={{ padding: "20px 16px" }}>
      <div style={{ textAlign: "center", marginBottom: "24px", padding: "24px", background: "rgba(23,23,23,0.95)", borderRadius: "16px", border: "1px solid rgba(255,255,255,0.08)" }}>
        <div style={{ fontSize: "64px" }}>ℹ️</div>
        <h2>Acerca de Conect Manzanillo</h2>
        <p>Plataforma comunitaria para monitoreo en tiempo real del tráfico portuario.</p>
      </div>
    </div>
  );
}
...
function CookieBanner() {
  ...
  useEffect(() => { if (!localStorage.getItem(COOKIE_KEY)) setShow(true); }, []);
  if (!show) return null;
  return (
    <div style={{ position: "fixed", bottom: 0, width: "100%", padding: "20px", background: "rgba(13,13,13,0.95)", borderTop: "1px solid #333", zIndex: 9999 }}>
      <button onClick={() => { localStorage.setItem(COOKIE_KEY, "true"); setShow(false); }} style={{ width: "100%", padding: "10px", background: "#3b82f6", color: "#fff", border: "none", borderRadius: "8px" }}>Aceptar Cookies</button>
    </div>
  );
}
...
function DonateBanner({ active }) {
  ...
  return (
    <div style={{ position: "fixed", bottom: "80px", left: "50%", transform: "translateX(-50%)", width: "90%", background: "rgba(16,185,129,0.2)", padding: "15px", borderRadius: "12px", border: "1px solid #10b981", textAlign: "center" }}>
      <p style={{ color: "#10b981", margin: "0 0 10px 0" }}>¿Te gusta la app? ☕</p>
      <a href="#" style={{ color: "#fff", textDecoration: "none", fontWeight: "bold" }}>Donar en PayPal</a>
    </div>
  );
}

function IncidentReportForm() {
  const [formData, setFormData] = useState({ acceso_id: "", tipo: "", descripcion: "", severidad: "media" });
  const supabase = getSupabase();
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!supabase) return;
    await supabase.from("incidents").insert([{ ...formData, reportado_en: new Date().toISOString() }]);
    alert("Reporte enviado");
  };
  return (
    <div style={{ padding: "20px" }}>
      <form onSubmit={handleSubmit} style={{ background: "#171717", padding: "20px", borderRadius: "12px" }}>
        <h3 style={{ textAlign: 'center' }}>Reportar Incidente</h3>
        <input style={{ width: "100%", marginBottom: "10px", padding: "10px" }} placeholder="Descripción" onChange={e => setFormData({ ...formData, descripcion: e.target.value })} />
        <button type="submit" style={{ width: "100%", padding: "10px", background: "#3b82f6", color: "#fff", border: "none" }}>Enviar</button>
      </form>
    </div>
  );
}
...
function App() {
  const { isAdmin, handleLogoTap, logout, AdminModal } = useAdminMode();
  ...
  const handleUserLogin = (user) => { setCurrentUser(user); setIsUserLoggedIn(true); };
  const handleUserLogout = () => { localStorage.removeItem(USER_KEY); setCurrentUser(null); setIsUserLoggedIn(false); };

  const handleVote = async (accesoId, retornoId) => {
    if (!supabase) return;
    try {
      await supabase.from("votes").insert([{ acceso_id: accesoId, retorno_id: retornoId, created_at: new Date().toISOString() }]);
    } catch (err) { console.error("Error voting:", err); }
  };

  const handleAdminStatusChange = (accesoId, status) => {
    setAdminStatuses((prev) => ({ ...prev, [accesoId]: status || undefined }));
  };

  return (
    <div style={{ minHeight: "100vh", background: "#0d0d0d", color: "#fff", fontFamily: "'Space Mono', monospace", paddingBottom: active === "info" ? "120px" : "20px" }}>
      <NavBar active={active} setActive={setActive} isAdmin={isAdmin} onLogoTap={handleLogoTap} />

      <div style={{ maxWidth: "480px", margin: "0 auto" }}>
        {active === "trafico" && (
          <div style={{ padding: "20px 16px" }}>
            <div style={{ textAlign: "center", marginBottom: "24px", padding: "20px", background: "rgba(23,23,23,0.95)", borderRadius: "16px", border: "1px solid rgba(255,255,255,0.08)" }}>
              <div style={{ fontSize: "48px" }}>🚦</div>
              <h2>Accesos Principales</h2>
              <p>Vota en tiempo real • Actualización automática</p>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
              {ACCESOS.map((acceso) => (
                <AccesoCard key={acceso.id} acceso={acceso} retornos={ACCESO_RETORNOS} estatusConfig={ESTATUS_CONFIG} onVote={handleVote} isAdmin={isAdmin} adminStatus={adminStatuses[acceso.id]} onAdminStatusChange={handleAdminStatusChange} />
              ))}
            </div>
            <TerminalesStatus />
          </div>
        ...
        <FloatingAuthButton onOpenAuth={() => setAuthOpen(true)} isLoggedIn={isUserLoggedIn} user={currentUser} onLogout={handleUserLogout} />
        {authOpen && <AuthModal onClose={() => setAuthOpen(false)} onLogin={handleUserLogin} />}
      </div>
      {AdminModal}
    </div>
  );
